...
<v-container fluid>
  ...
  <header-small-device :drawer="drawerVisible" @toggle-drawer="toggleDrawer" class="hidden-lg-and-up"/>
  <v-layout>
    <v-navigation-drawer v-model="drawerVisible" temporary>
      ...
      <v-list>
        <v-list-item v-for="link in navLinks" :key="link.to" link :to="link.to">
          <v-list-item-title @click="startDownload(link)" :style="{fontSize: '20px'}">{{ link.text }}</v-list-item-title>
        </v-list-item>
      </v-list>
